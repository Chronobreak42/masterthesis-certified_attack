...
class LinkPredictionGNN(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim):
        super().__init__()
        self.encoder = GNNEncoder(in_dim, hidden_dim, out_dim)
        self.edge_head = EdgeMLP(out_dim)

    def forward(self, x, edge_index_struct, edge_index_lab):
        """
        x:                (N, F)
        edge_index_struct:(2, E) used for GNN message passing
        edge_index_lab:   (2, M) pairs to classify (existing OR non-existing)
        """
        h = self.encoder(x, edge_index_struct)          # (N, d)
        logits = self.edge_head(h, edge_index_lab)      # (M, 1)
        return logits